...
const genComponentWithQuery = (name, method, path, style, autoRefreshSec, query) => {
  const component = genComponent(name, method, path, style, autoRefreshSec);
  if (query) {
  ...
  const result = {
    ...
    pages: [].concat(
      ...
      genSection(constant.VIRON_SECTION_MANAGE, {
        ...
        // Manage
        [constant.GROUP_DAILY]: [
          {
            id: 'rep_daily_charge',
            name: 'all',
            components: [
              genComponent('Chart(課金数)', 'get', '/rep_daily_charge/count', constant.VIRON_STYLE_GRAPH_STACKED_BAR),
              genComponent('Chart(課金額)', 'get', '/rep_daily_charge/price', constant.VIRON_STYLE_GRAPH_STACKED_BAR),
              genTableComponent('従量サマリ', 'get', '/rep_daily_charge', 'site_id', [{key: 'site_id', type: 'string'}], ['site_id']),
            ]
          },
          {
            id: 'rep_daily_charge_getters',
            name: 'getters',
            components: [
              // genComponent('Chart(課金数)', 'get', '/rep_daily_charge/getters/count', constant.VIRON_STYLE_GRAPH_STACKED_BAR),
              // genComponent('Chart(課金額)', 'get', '/rep_daily_charge/getters/price', constant.VIRON_STYLE_GRAPH_STACKED_BAR),
              genComponent('課金数(今月)', 'get', '/rep_daily_charge/getters/count/this?site_id=getters&period_type=this', constant.VIRON_STYLE_GRAPH_STACKED_BAR),
              genComponent('課金数(先月)', 'get', '/rep_daily_charge/getters/count/last', constant.VIRON_STYLE_GRAPH_STACKED_BAR),
              genComponent('課金額(今月)', 'get', '/rep_daily_charge/getters/price/this', constant.VIRON_STYLE_GRAPH_STACKED_BAR),
              genComponent('課金額(先月)', 'get', '/rep_daily_charge/getters/price/last', constant.VIRON_STYLE_GRAPH_STACKED_BAR),
              genTableComponent('従量サマリ', 'get', '/rep_daily_charge', 'site_id', [{key: 'site_id', type: 'string'}], ['site_id']),
            ]
          },
          {
